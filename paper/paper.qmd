---
title: "Defending Against Manipulation: The Impact of Psychological Inoculation on Social Media Discernment"
subtitle: "An Analysis of Technique Discernment and Demographic Influences"
author: 
  - Elizabeth Luong
thanks: "Special thanks to Rohan Alexander for his guidance. Data and code are available at https://github.com/Luongel1/psychological-inoculation.git."
date: today
date-format: long
abstract: >
  Social media is increasingly used to spread manipulative and deceptive content, posing risks to informed public discourse. This paper explores whether psychological inoculation—a strategy akin to a "vaccine" for misinformation—can improve users' ability to identify manipulative techniques on social media, termed "technique discernment." Using Bayesian regression models and survey data, the study finds that individuals exposed to inoculation treatments scored significantly higher in discerning manipulative content compared to those in control groups, even when accounting for demographic and psychological differences. These results demonstrate the potential of inoculation strategies to build resilience against misinformation, providing practical guidance for designing interventions in digital spaces.
format:
  pdf:
    toc: true
    number-sections: true
bibliography: references.bib
---

```{r}
#| include: false
#| warning: false
#| message: false

library(tidyverse)
library(here)
library(rstanarm)
library(modelsummary)
library(ggplot2)
library(kableExtra)
library(arrow)
library(gridExtra)
library(knitr)
library(car)
library(dplyr)
library(stringr)
library(magrittr)
library(tibble)
library(bayesplot)
library(broom.mixed)


cleaned_data <- read_parquet(here::here("data/02-analysis_data/analysis_data.parquet"))

```

# Introduction

Social media's influence on public opinion has raised growing concerns about the spread of misinformation and manipulative content. These platforms enable the rapid dissemination of misleading narratives that shape beliefs, decisions, and behaviors, posing challenges to informed decision-making and democratic processes. Addressing these concerns is essential for fostering a resilient and well-informed public.

Psychological inoculation offers a proactive solution by equipping individuals with the cognitive tools to resist manipulation before encountering it. Similar to a vaccine, inoculation introduces individuals to weakened forms of manipulative techniques, thereby building their resistance to such tactics in real-world scenarios. This study investigates whether inoculation can enhance "technique discernment," or the ability to distinguish manipulative from neutral content on social media.

The estimand of this study is the average treatment effect (ATE) of inoculation on technique discernment scores. This represents the difference in discernment ability between individuals exposed to inoculation treatments and those in control conditions, measured across a representative sample.

Using data from a randomized survey experiment, participants were assigned to either inoculation or control conditions and evaluated on their ability to identify manipulative and neutral content. Bayesian regression models were applied to estimate the treatment effect while controlling for demographic and psychological factors, such as age, gender, education, political ideology, and cognitive traits. The findings show a significant positive effect of inoculation on discernment scores, underscoring its potential to strengthen resistance to misinformation. Further analysis highlights how demographic and psychological characteristics moderate the intervention's effectiveness.

The results show a significant positive impact of inoculation on discernment scores, demonstrating the potential for preemptive interventions to bolster resistance to misinformation. Furthermore, analysis of demographic and psychological covariates shows the differential effectiveness of the intervention across subgroups.

The remainder of this paper is structured as follows: @sec-data details the data and methods used, @sec-model describes the statistical approach and Bayesian modeling framework, @sec-results presents the findings with supporting visualizations, and @sec-discussion concludes with implications and recommendations for addressing the challenges of misinformation on social media.


# Data {#sec-data}

The data for this study is analyzed using the statistical programming language R [@citeR], using an extensive suite of packages for data cleaning, analysis, visualization, and modeling. The primary packages include `tidyverse` [@citeTidyverse] for data manipulation and visualization, `arrow` [@citeArrow] for efficient data storage and retrieval, `ggplot2` [@citeGgplot2] for creating detailed visualizations, and `rstanarm` [@rstanarm] for Bayesian regression modeling. Additional libraries used are here for managing file paths, `modelsummary` [@citeModelSummary] for streamlined model summaries, `kableExtra` [@citeKableExtra] for formatting polished tables, `gridExtra` and `knitr` for
visualization and reporting, and car for regression diagnostics such as VIF calculation.

For statistical modeling and reporting, `bayesplot` and `broom.mixed` facilitate Bayesian posterior checks and tidy summaries of model outputs, respectively. `dplyr`, `stringr`, and `tibble` are employed for data manipulation and string handling, while magrittr enables efficient piping operations. This diverse set of packages ensures a robust analytical workflow for investigating the effects of psychological inoculation.

These tools enable the analysis of a dataset focused on psychological inoculation, specifically its effect on participants' ability to discern manipulative techniques in social media content. The data preparation and analysis pipeline ensures robust handling of variables and addresses potential sources of bias, making the results both reliable and interpretable.


## Data Source
	
The data for this study was sourced from a controlled experimental design hosted on the Open Science Framework (OSF) platform, accessible at https://osf.io/3769y/. The experiment examined the effects of psychological inoculation on individuals' ability to identify manipulative content on social media. Participants were recruited to represent diverse demographic, psychological, and behavioral profiles, ensuring a well-rounded and robust dataset.

Data collection followed strict ethical protocols, including obtaining informed consent and ensuring participant confidentiality. Participants were randomly assigned to inoculation or control groups and completed a series of tasks assessing their discernment of manipulative and neutral social media posts. Additionally, demographic factors such as age, gender, education, and political ideology, along with psychological traits, were recorded to enable detailed analysis of covariate effects.

This dataset is particularly suitable for evaluating the impact of inoculation interventions due to its design, which captures the treatment's efficacy and explores its relationship with various demographic and psychological variables. The structured and transparent methodology ensures reliability, making this dataset an ideal resource for testing the proposed hypotheses.

## Data Cleaning and Variables

The dataset used in this study underwent extensive cleaning to ensure the inclusion of only relevant variables and high-quality data for the analysis. The raw data was read from the "Combined" tab of OSF's 'Studies 1-5 datasets' and systematically processed to address missing values, renamed variables for clarity, and standardize the dataset for meaningful analysis. This process resulted in a structured dataset with clearly defined variables essential for exploring the relationship between psychological inoculation and participants’ ability to discern manipulative content on social media.

The cleaning process started by filtering out rows with missing values, such as `Condition`, `Diff-Technique`, `Age-text`, `Gender-text`, `Education`, `Political-Ideology`, `Bullshit-Receptivity`, `Conspiracy-Belief`, and `Analytical-Thinking`. The rationale for this step was to ensure that all variables of interest had complete information, enabling meaningful analysis.

To facilitate analysis, key variables were transformed and standardized:

- **Treatment Assignment**: The `Condition` variable, indicating whether a participant was part of the inoculation or control group, was transformed into a binary categorical variable named `treatment` with levels "Inoculation" and "Control."
- **Demographics**: The `Gender-text` variable was standardized to include consistent categories such as "Male," "Female," "Non-binary," "Prefer not to say," and "Other." The `Education` variable was similarly mapped to meaningful categories like "High School," "Bachelor's," "Master's," and "Doctorate."
- **Age Representation**: The `Age-text` variable was retained as a character variable to represent age ranges.
- **Psychological Variables**: Key psychological variables, including `Analytical-Thinking`, `Conspiracy-Belief`, and `Bullshit-Receptivity`, were standardized using z-scores to facilitate comparison.
- **Political Ideology**: The `Political-Ideology` variable was recoded into meaningful categories ("Liberal," "Moderate," "Conservative") based on its original numeric levels.

The final cleaned dataset included the following variables:
`treatment`, `technique_discernment`, `standardized_analytical_thinking`,
`standardized_conspiracy_belief`, `standardized_bullshit_receptivity`, `gender`, `education`, `political_ideology`, and `age_text.`

The cleaned dataset was summarized to provide an overview of its structure and distribution. @tbl-clean-data provides a preview of the first five rows of the dataset, highlighting key variables across treatment groups, psychological measures, and demographics.
```{r}
#| label: tbl-clean-data
#| tbl-cap: "Preview of the cleaned OSF dataset"
#| echo: false
#| warning: false
#| message: false

# Preview of the first few rows of the dataset with cleaned column names
cleaned_data %>%
  slice_head(n = 5) %>%
  select(
    treatment, technique_discernment, standardized_analytical_thinking,
    standardized_conspiracy_belief, standardized_bullshit_receptivity,
    gender, education, political_ideology, age_text
  ) %>%
  rename_with(~ stringr::str_replace_all(., "_", " ")) %>% # Replace underscores with spaces in column names
  kable(booktabs = TRUE, linesep = "", longtable = TRUE) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),
    full_width = FALSE,
    font_size = 7, # Slightly reduce font size for better fit
    position = "center"
  ) %>%
  add_header_above(
    c(" " = 1, "Psychological Variables" = 4, "Demographics" = 4) # Correct grouping for all 9 columns
  ) %>%
   column_spec(1, width = "1.8cm", bold = TRUE) %>% # Adjust column widths for clarity
  column_spec(2:5, width = "1.5cm") %>% # Narrower columns for Psychological Variables
  column_spec(6:9, width = "1.8cm") %>% # Adjust for Demographics
  row_spec(0, bold = TRUE, background = "#f7f7f7") # Header row styling

```

```{r}
#| label: tbl-categorial
#| tbl-cap: "Statistics summary of the categorical variables in cleaned OSF dataset"
#| echo: false
#| warning: false
#| message: false
#| fig-height: 5 
#| fig-width: 15

# Summarize categorical variables and combine into one data frame
categorical_summary <- bind_rows(
  cleaned_data %>%
    count(treatment) %>%
    rename(Category = treatment, Frequency = n) %>%
    mutate(Variable = "Treatment"),
  
  cleaned_data %>%
    count(gender) %>%
    rename(Category = gender, Frequency = n) %>%
    mutate(Variable = "Gender"),
  
  cleaned_data %>%
    count(education) %>%
    rename(Category = education, Frequency = n) %>%
    mutate(Variable = "Education"),
  
  cleaned_data %>%
    count(political_ideology) %>%
    rename(Category = political_ideology, Frequency = n) %>%
    mutate(Variable = "Political Ideology"),
  
  cleaned_data %>%
    count(age_text) %>%
    rename(Category = age_text, Frequency = n) %>%
    mutate(Variable = "Age Group")
) %>%
  arrange(Variable, desc(Frequency)) # Order by variable and frequency

# Format table to show variable headers with their respective details listed beneath
formatted_table <- categorical_summary %>%
  group_by(Variable) %>%
  mutate(
    Variable = if_else(row_number() == 1, Variable, "") # Show Variable name only once
  ) %>%
  ungroup() %>%
  select(Variable, Category, Frequency) # Select the relevant columns

# Create a well-formatted hierarchical table using kableExtra
formatted_table %>%
  kbl(
    booktabs = TRUE,
    col.names = c("Variable", "Category", "Frequency"),
    caption = "Summary of Categorical Variables"
  ) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),
    full_width = FALSE,
    position = "center",
    font_size = 12
  ) %>%
  column_spec(1, bold = TRUE, width = "10em") %>% # Bold and adjust width for Variable column
  column_spec(2, width = "12em") %>% # Adjust width of Category column
  column_spec(3, width = "8em") %>% # Adjust width of Frequency column
  row_spec(0, bold = TRUE, background = "#f2f2f2") # Style header row

```

```{r}
#| label: tbl-numeric
#| tbl-cap: "Statistics summary of the numeric variables in cleaned OSF dataset"
#| echo: false
#| warning: false
#| message: false

# Summary statistics for numeric variables
numeric_summary <- cleaned_data %>%
  select(
    technique_discernment,
    standardized_analytical_thinking,
    standardized_conspiracy_belief,
    standardized_bullshit_receptivity
  ) %>%
  summary()

# Rename variables to replace underscores with spaces
colnames(numeric_summary) <- stringr::str_replace_all(
  colnames(numeric_summary),
  "_",
  " "
)

# Render the table with adjusted formatting
numeric_summary %>%
  kable(
    booktabs = TRUE,
    linesep = "",
    longtable = TRUE, # Enables table spanning multiple pages
    align = "c" # Center-align contents
  ) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),
    full_width = TRUE, # Ensures the table doesn't stretch
    font_size = 10, # Adjust font size for better readability
    position = "center" # Centers the table on the page
  ) %>%
  add_header_above(
    c(" " = 1, "Numeric Variables Summary" = 4) # Correct grouping for the header
  ) %>%
  column_spec(1, bold = TRUE, width = "4cm") %>% # Adjust and bold the first column
  column_spec(2:5, width = "3cm") %>% # Adjust widths for numeric columns
  row_spec(0, bold = TRUE, background = "#f7f7f7") # Style the header row


```

@tbl-categorial demonstrates a balanced experimental design between the treatment groups, with 2,757 participants in the Control group and 2,748 in the Inoculation group. Gender distribution reflects a higher representation of females (2,751) and males (2,685), alongside smaller groups of non-binary individuals (52) and those preferring not to disclose their gender (7). Regarding education, the data is skewed towards participants with post-graduate degrees, as evidenced by 2,306 participants holding Doctorates and 1,709 with Master's degrees, whereas only a small fraction reported High School (37) or Bachelor's education (149). Political ideology distribution shows considerable diversity: 2,201 participants identify as Liberal, 2,147 as Conservative, and 1,157 as Moderate. Age distribution spans a broad range, with the largest cohort being participants aged 55 or older (1,528) and the smallest being those aged 18–24 (707).

@tbl-numeric complements the categorical summary by providing key statistics about continuous variables. For `technique_discernment`, scores range widely from -4.89 to 6.00, with a mean of 1.83, indicating that participants’ ability to discern information varies greatly. The first quartile (0.50) and median (1.83) suggest a central tendency around a moderately positive discernment score.

Similarly, `standardized_analytical_thinking` has a mean of 0, consistent with standardization, and ranges from -1.16 to 1.76. The third quartile value (0.79) indicates that a significant portion of participants scored above average in analytical thinking.

For `standardized_conspiracy_belief`, the mean of 0 and a range of -2.77 to 1.79 show a distribution where most participants lean slightly negative or neutral, as reflected in the median (0.06). `Standardized_bullshit_receptivity` displays a similar pattern, with scores ranging from -1.81 to 2.44, a mean of 0, and a median of 0.10, suggesting slight receptivity for many participants but significant variability across the population.

## Data Measurment

Because the data for this research was collected through surveys, it is essential to acknowledge the potential for inconsistencies, biases, and misinterpretations in the respondents' answers. This section examines what the variables are intended to measure and identifies any potential issues with their interpretation or accuracy.

The psychological variables, including `Diff-Technique` (representing technique discernment), `Bullshit-Receptivity`, `Conspiracy-Belief`, and `Analytical-Thinking`, are measured using self-reported scales. These measures depend heavily on respondents’ honest and consistent self-assessments. For example, respondents may overstate their analytical thinking abilities or understate their susceptibility to conspiratorial beliefs due to social desirability bias. Furthermore, while these variables were standardized for comparability, it is essential to note that standardization assumes a normal distribution of the underlying constructs, which may not hold true for all measures.

Demographic variables such as `Gender-text`, `Education`, and `Age-text` provide contextual information but are also subject to issues of self-reporting. For instance, the `Gender-text` variable captures a wide range of gender identities, including "Non-binary" and "Prefer not to say." While this inclusivity is commendable, it introduces variability in the dataset, particularly when group sizes are small, which could limit statistical power in subgroup analyses.

Similarly, the `Education` variable categorizes respondents into levels such as "High School," "Bachelor's," "Master's," and "Doctorate." While these categories provide a clear stratification of educational attainment, there may be discrepancies in how respondents interpret their educational qualifications, particularly for international participants whose education systems may not align with these classifications. Missing values in this variable could also indicate respondents' reluctance to disclose their educational background.

The `Age-text` variable represents respondents' age groups, providing a straightforward breakdown into categories such as "18-24," "25-34," and "55 or older." These categories offer a broad overview but may overlook finer gradations in age that could influence psychological or behavioral outcomes.

The variable `Political-Ideology` categorizes respondents into "Liberal," "Moderate," and "Conservative." While these categories simplify analysis, they may mask the variation of individuals' political beliefs, particularly for those who hold mixed or unconventional ideologies. Additionally, respondents' self-reported political alignment might be influenced by social desirability or recall bias, particularly in polarized contexts where identifying with one ideology may carry stigma.

Like many survey-based studies, this dataset is subject to recall bias, social desirability bias, and potential non-response biases. For instance, participants may provide responses that they perceive as socially acceptable rather than those that truly reflect their beliefs or behaviors. Furthermore, missing data in key variables such as `Education` and `Political-Ideology` could lead to biased estimates if the missingness is not random.

Despite these challenges, the dataset provides a rich foundation for exploring the effects of psychological inoculation on technique discernment while accounting for a diverse range of demographic and psychological variables. Careful interpretation and robust statistical methods will help mitigate these limitations in subsequent analyses.


## Data Analysis

The psychological inoculation approach aims to improve individuals' ability to discern techniques used in misinformation. To understand the treatment's impact, the distribution of `technique_discernment` was examined for the control and inoculation groups. As shown in @fig-technique, the inoculation group has a slightly higher mean discernment score compared to the control group, suggesting potential effectiveness. However, overlap in distributions indicates that further modeling is required to establish statistical significance.

```{r}
#| label: fig-technique
#| fig-cap: Distribution of Technique Discernment by Treatment Groups
#| echo: false
#| warning: false
#| message: false
#| fig-height: 8 
#| fig-width: 10

ggplot(cleaned_data, aes(x = treatment, y = technique_discernment, fill = treatment)) +
  geom_boxplot(alpha = 0.7, outlier.shape = NA) +
  scale_fill_brewer(palette = "Set2") +
  labs(title = "Technique Discernment by Treatment", x = "Treatment Group", y = "Technique Discernment") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 20, face = "bold"),
    axis.text.x = element_text(size = 18),
    axis.text.y = element_text(size = 18),
    axis.title.y = element_text(size = 20),
    axis.title.x = element_text(size = 20)
  )

```

Ensuring balance across demographic variables is essential for isolating treatment effects. @fig-demographics illustrates the distribution of key demographic variables across treatment groups. Gender representation appears balanced, with similar proportions of male, female, and non-binary participants in both groups. Education and political ideology also display reasonable distributions, mitigating concerns of systematic imbalances.

```{r}
#| label: fig-demographics
#| fig-cap: Distribution of Key Demographic Variables Across Treatment groups
#| echo: false
#| warning: false
#| message: false
#| fig-height: 7 
#| fig-width: 15


plot_gender <- ggplot(cleaned_data, aes(x = treatment, fill = gender)) +
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_brewer(palette = "Set2") +
  labs(
    title = "Gender Distribution",
    x = "Treatment Group",
    y = "Proportion"
  ) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size = 15),
    axis.text.y = element_text(size = 18),
    plot.title = element_text(size = 20, face = "bold"),
    legend.title = element_text(size = 18),
    legend.text = element_text(size = 18),
    axis.title.y = element_text(size = 20),
    axis.title.x = element_text(size = 20)
  )

plot_education <- ggplot(cleaned_data, aes(x = treatment, fill = education)) +
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_brewer(palette = "Set3") +
  labs(
    title = "Education Distribution",
    x = "Treatment Group",
    y = "Proportion"
  ) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size = 15),
    axis.text.y = element_text(size = 18),
    plot.title = element_text(size = 20, face = "bold"),
    legend.title = element_text(size = 18),
    legend.text = element_text(size = 18)
  )

plot_ideology <- ggplot(cleaned_data, aes(x = treatment, fill = political_ideology)) +
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_brewer(palette = "Dark2") +
  labs(
    title = "Political Ideology Distribution",
    x = "Treatment Group",
    y = "Proportion"
  ) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size = 15),
    axis.text.y = element_text(size = 18),
    plot.title = element_text(size = 20, face = "bold"),
    legend.title = element_text(size = 18),
    legend.text = element_text(size = 18)
  )

# Arrange the plots in a single row with spacing adjustments
grid.arrange(
  plot_gender, plot_education, plot_ideology,
  ncol = 3,
  widths = c(1.1, 1.1, 1.1) # Adjust widths for equal proportions
)

```

The standardized psychological variables: `standardized_analytical_thinking`,

`standardized_bullshit_receptivity`, and `standardized_conspiracy_belief`—are key metrics for evaluating cognitive and emotional responses to misinformation. @fig-psych compares these variables across treatment groups, showing slight but consistent differences favoring the inoculation group. For instance, the inoculation group shows lower susceptibility to conspiracy beliefs, aligning with the intended effects of the intervention.

```{r}
#| label: fig-psych
#| fig-cap: Comparing Standardized Variables Across Treatment Groups
#| echo: false
#| warning: false
#| message: false
#| fig-height: 8 
#| fig-width: 17

# Reshape data to long format
long_data <- cleaned_data %>%
  pivot_longer(
    cols = c(standardized_analytical_thinking, 
             standardized_bullshit_receptivity, 
             standardized_conspiracy_belief),
    names_to = "variable",
    values_to = "value"
  ) %>%
  mutate(variable = stringr::str_replace_all(variable, "_", " ")) # Replace underscores with spaces

# Plot using facets
ggplot(long_data, aes(x = treatment, y = value, fill = treatment)) +
  geom_violin(alpha = 0.7, color = "black") + # Add black outline for contrast
  labs(
    title = "Psychological Variables by Treatment", 
    x = "Treatment Group", 
    y = "Standardized Value"
  ) +
  scale_fill_brewer(palette = "Set2") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 34, face = "bold", hjust = 0.5), # Center and enlarge title
    axis.title.x = element_text(size = 21, face = "bold"), # Adjust x-axis label size
    axis.title.y = element_text(size = 21, face = "bold"), # Adjust y-axis label size
    axis.text.x = element_text(size = 21, angle = 45, hjust = 1), # Rotate and size x-axis text
    axis.text.y = element_text(size = 21), # Adjust y-axis text size
    strip.text = element_text(size = 18, face = "bold"),  # Adjust facet labels size
    legend.title = element_text(size = 18, face = "bold"), # Adjust legend title size
    legend.text = element_text(size = 18), # Adjust legend text size
    panel.grid.major = element_line(size = 0.5, linetype = "dashed", color = "gray") # Add subtle grid lines
  ) +
  facet_wrap(~ variable, scales = "free_y", nrow = 1) # Ensure facets are in a single row for better fit

```
To explore potential interaction effects between age and political ideology, @fig-age-ideology examines the relationship between these variables across treatment groups. Younger participants (ages 18-34) tend to lean liberal, while older participants are more evenly distributed across ideological categories. This interaction warrants further analysis to determine its implications for treatment effectiveness.
```{r}
#| label: fig-age-ideology
#| fig-cap: Age and Political Ideology Distribution Across Treatment Groups
#| echo: false
#| warning: false
#| message: false
#| fig-height: 5 
#| fig-width: 8

ggplot(cleaned_data, aes(x = age_text, fill = political_ideology)) +
  geom_bar(position = "fill") +
  facet_wrap(~ treatment) +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_brewer(palette = "Pastel1") +
  labs(
    title = "Age and Political Ideology Interaction", 
    x = "Age Group", 
    y = "Proportion"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 16, face = "bold", hjust = 0.5), # Center and bold title
    axis.title.x = element_text(size = 14, face = "bold"),           # Larger and bold x-axis title
    axis.title.y = element_text(size = 14, face = "bold"),           # Larger and bold y-axis title
    axis.text.x = element_text(size = 12, angle = 45, hjust = 1),    # Rotate x-axis labels for readability
    axis.text.y = element_text(size = 12),                           # Increase y-axis text size
    strip.text = element_text(size = 14, face = "bold"),             # Larger facet labels
    legend.title = element_text(size = 14, face = "bold"),           # Larger legend title
    legend.text = element_text(size = 12)                            # Larger legend text
  )

```

The visual analyses offer an initial understanding of the balance and variability of key variables across treatment groups. Differences in technique discernment and standardized psychological variables suggest potential treatment effects. However, the observed overlap and interaction effects highlight the need for multivariate modeling to disentangle causal relationships.

# Model {#sec-model}

This study employs a Bayesian linear regression model to investigate the effects of psychological inoculation on key standardized psychological measures, including analytical thinking, bullshit receptivity, and conspiracy belief. The model is structured as follows:

\begin{align}
y_i | \mu_i, \sigma^2 &\sim \text{Normal}(\mu_i, \sigma^2) \\
\mu_i &= \alpha + \beta_1 \times \text{Treatment}_i + \beta_2 \times \text{Gender}_i + \beta_3 \times \text{Education}_i + \beta_4 \times \\                              
\text{Political Ideology}_i
+ \beta_5 \times \text{Age Group}_i \\
\alpha &\sim \text{Normal}(0, 2.5) \\
\beta_j &\sim \text{Normal}(0, 2.5), \quad j = 1, 2, ..., 5
\end{align}

In this model:
\begin{itemize}
  \item \( y_i \): Represents the outcome variable, one of the standardized psychological measures: analytical thinking, bullshit receptivity, or conspiracy belief.
  \item \( \mu_i \): The expected value of the outcome for individual \( i \), modeled as a linear combination of the intercept (\( \alpha \)) and coefficients (\( \beta_1, \beta_2, ..., \beta_5 \)) corresponding to the predictor variables.
  \item Predictor variables include:
  \begin{itemize}
    \item \( \text{Treatment}_i \): Indicates whether the individual was in the inoculation or control group.
    \item \( \text{Gender}_i \): Categorical variable indicating gender.
    \item \( \text{Education}_i \): Categorical variable capturing education level.
    \item \( \text{Political Ideology}_i \): Categorical variable representing political ideology (e.g., liberal, moderate, conservative).
    \item \( \text{Age Group}_i \): Categorical variable grouping individuals by age.
  \end{itemize}
\end{itemize}

## Priors

Each parameter is assigned a weakly informative prior. The intercept and coefficients ( $\alpha$, $\beta_j$) are modeled using a Normal distribution with mean 0 and standard deviation 2.5 to allow flexibility while avoiding extreme parameter estimates. This regularization improves interpretability and stabilizes estimation, particularly with categorical predictors.

## Modeling Approach

Bayesian methods are particularly advantageous for this analysis, as they allow for the incorporation of prior information and produce a full posterior distribution for each parameter, enabling robust inference. This approach is particularly relevant in studies like psychological inoculation, where prior research can provide important guidance on expected effects.

Alternative approaches, such as frequentist regression, were considered but ultimately not adopted due to their limitations in handling uncertainty and prior information. The Bayesian framework enables a richer understanding of the data and more robust parameter estimation.

### Implementation

The Bayesian regression model is implemented in R [@citeR] using the `rstanarm` package [@rstanarm]. The `rstanarm` package uses Markov Chain Monte Carlo (MCMC) sampling to estimate the posterior distributions of the parameters. The default priors in `rstanarm` are employed for this analysis, which align with the weakly informative priors described above.

### Sampling

To optimize computational efficiency, a random sub-sample of 3000 observations is used to fit the model. This ensures manageable run-time without sacrificing the representation of the data.

### Justification for Model Choice

The linear regression framework is suitable for continuous outcome variables, such as the standardized psychological measures in this study. Bayesian methods enhance the model’s robustness by accounting for uncertainty in parameter estimation. Furthermore, the inclusion of demographic covariates (gender, education, political ideology, and age group) ensures that the effects of treatment are examined while controlling for potential confounding factors. This approach strengthens the validity of the findings, providing a solid foundation for subsequent interpretation and discussion.


## Model Justification

The choice of a Bayesian regression model for this study reflects the need to analyze the complex relationships between psychological measures, demographic variables, and treatment groups while accounting for uncertainty and potential confounding factors. Psychological inoculation theory posits that exposure to counterarguments or "inoculation treatments" strengthens resistance to misinformation and enhances critical-thinking abilities. As such, a positive relationship is expected between being in the inoculation group and higher analytical thinking scores, as well as lower receptivity to bullshit statements and conspiracy beliefs. This expectation aligns with prior research demonstrating the efficacy of inoculation techniques in bolstering cognitive resistance to manipulative messaging [@banas2011inoculation; @roozenbeek2020inoculation].

Demographic variables such as gender, education, political ideology, and age group are included in the model as covariates to control for their potential influence on the psychological measures. For example, individuals with higher levels of education are hypothesized to exhibit greater analytical thinking and reduced receptivity to conspiratorial or nonsensical claims due to their exposure to critical-reasoning frameworks during formal education. Similarly, political ideology may influence these outcomes, as previous studies suggest that ideological beliefs can shape susceptibility to misinformation and receptivity to conspiratorial narratives [@van2018conspiracy].

Gender and age group are also anticipated to play roles in psychological outcomes. Prior literature indicates that gender differences may exist in analytical thinking and susceptibility to certain cognitive biases, with some studies suggesting that men may score slightly higher on measures of analytical reasoning, whereas women may exhibit greater emotional intelligence and skepticism [@baron2000thinking]. Age, on the other hand, may influence individuals' familiarity with certain misinformation tactics, with younger individuals potentially being more attuned to online misinformation but older individuals potentially relying more on traditional media sources.

In terms of treatment effects, it is hypothesized that individuals in the inoculation group will demonstrate higher scores on analytical thinking and lower scores on bullshit receptivity and conspiracy beliefs compared to the control group. This aligns with the theoretical framework of psychological inoculation, which posits that pre-exposure to weakened forms of misinformation equips individuals with cognitive tools to resist subsequent manipulative messaging [@mcguire1964inducing].

The Bayesian modeling approach is justified by its ability to incorporate prior knowledge and produce credible intervals for parameter estimates, offering a richer and more interpretable framework for hypothesis testing compared to frequentist methods. Moreover, Bayesian analysis provides flexibility in modeling categorical variables and controlling for demographic covariates, ensuring robust estimation of treatment effects. By explicitly modeling uncertainty, this approach enhances the reliability of the findings, particularly in the context of survey-based data where measurement error and sampling variability may be present.

In summary, the Bayesian regression model is well-suited to address the research questions of this study, utilizing its capacity to incorporate prior knowledge, handle complex relationships, and provide robust estimates of treatment effects while accounting for demographic variables. The model's structure and the inclusion of key covariates reflect a theoretically grounded approach to understanding the impact of psychological inoculation on critical-thinking and resistance to misinformation.


# Results {#sec-results}

## Model Summary

The Bayesian linear regression models, developed to assess the effects of psychological inoculation on analytical thinking, bullshit receptivity, and conspiracy belief, show casing several notable findings. The results are summarized in @tbl-modelresults.

```{r}
#| label: tbl-modelresults
#| fig-cap: Summary of Bayesian Linear Regression on the Effects of Psychological Inoculation 
#| echo: false
#| eval: true
#| warning: false
#| message: false
#| fig-height: 5
#| fig-width: 15

# Load the saved model
psychological_model <- readRDS(file = here::here("models/psych_inoculation_model.rds"))

# Fix any variable name issues and extract a clean summary
# Use `broom.mixed::tidy()` to extract the summary as a dataframe
model_summary <- broom.mixed::tidy(psychological_model, conf.int = TRUE)

# Generate a model summary table
model_summary_table <- model_summary %>%
  dplyr::select(term, estimate, std.error, conf.low, conf.high) %>%
  dplyr::mutate(
    term = stringr::str_replace_all(term, "_", " "), # Format terms for readability
    conf.int = paste0("(", round(conf.low, 2), ", ", round(conf.high, 2), ")")
  ) %>%
  dplyr::select(term, estimate, std.error, conf.int)

model_summary_table %>%
  kbl(
    caption = "Psychological Inoculation Model Results",
    col.names = c("Predictor", "Estimate", "Standard Error", "95% CI")
  ) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),
    full_width = FALSE
  )

```
@tbl-modelresults provides the posterior mean estimates and median absolute deviation (MAD) of the coefficients for each predictor. The intercept represents the expected outcome when all predictors are at their reference levels. For example, the baseline levels include individuals in the control group, identifying as "Male," with "High School" education, "Moderate" political ideology, and belonging to the "18–24" age group.

### Analytical Thinking
The results indicate a statistically significant positive effect of the inoculation treatment on standardized analytical thinking scores. Participants in the inoculation group demonstrate higher analytical thinking compared to the control group. The coefficient for the treatment variable is positive, indicating a meaningful increase in analytical thinking.

### Bullshit Receptivity
For bullshit receptivity, the results suggest a negative relationship between inoculation treatment and receptivity to misleading information. Individuals in the inoculation group exhibit lower bullshit receptivity scores, implying that the treatment enhances their ability to discern and reject implausible or deceptive claims.

### Conspiracy Belief
The model examining conspiracy beliefs shows that inoculation treatment reduces susceptibility to conspiracy theories. The negative coefficient for the treatment variable suggests a statistically significant decline in conspiracy belief scores among treated participants.

### Demographic Effects
Gender, education, political ideology, and age group are included as covariates to control for demographic differences:

- **Gender**: There is no substantial difference in psychological measures between genders, except for a slight increase in analytical thinking for females.
- **Education**: Higher education levels, such as Bachelor's or Master's degrees, are positively associated with analytical thinking and negatively associated with bullshit receptivity and conspiracy belief.
- **Political Ideology**: Conservative participants tend to score higher on bullshit receptivity and conspiracy belief compared to their liberal counterparts.
- **Age Group**: Older age groups generally exhibit lower scores for bullshit receptivity and conspiracy belief, while analytical thinking increases with age.
Credibility Intervals

@fig-modelresults1 illustrates the 90% credibility intervals for the coefficients, highlighting the range of likely values for each estimate. Coefficients with credibility intervals that do not cross zero are statistically significant.

```{r}
#| label: fig-modelresults1
#| fig-cap: Credibility Intervals for Coefficients
#| echo: false
#| warning: false
#| message: false
#| fig-height: 5 
#| fig-width: 10

# Extract posterior intervals (90% credible intervals) for coefficients only
posterior_interval <- as.data.frame(posterior_interval(psychological_model, prob = 0.9))

# Subset only rows matching the coefficients
coefficients <- coef(psychological_model)
posterior_interval <- posterior_interval[rownames(posterior_interval) %in% names(coefficients), ]

# Add point estimates for each predictor
posterior_interval <- cbind(
  Predictor = rownames(posterior_interval),
  Estimate = coefficients,
  posterior_interval
)

# Rename columns for clarity
colnames(posterior_interval) <- c("Predictor", "Estimate", "Lower", "Upper")

# Clean up predictor names: Add colons and fix variable formatting
posterior_interval$Predictor <- posterior_interval$Predictor %>%
  gsub("_", " ", .) %>%                       # Replace underscores with spaces
  gsub("gender(\\w+)", "gender: \\1", .) %>% # Format gender variables
  gsub("education(\\w+)", "education: \\1", .) %>% # Format education variables
  gsub("political ideology(\\w+)", "political ideology: \\1", .) %>% # Format political ideology
  gsub("age text(\\d+-\\d+|55 or older)", "age: \\1", .) %>% # Format age variables
  gsub("standardized analytical thinking", "standardized analytical thinking", .) %>%
  gsub("standardized conspiracy belief", "standardized conspiracy belief", .) %>%
  gsub("standardized bullshit receptivity", "standardized bullshit receptivity", .) %>%
  gsub("\\(Intercept\\)", "Baseline: Intercept", .) # Adjust intercept formatting

# Capitalize certain values, e.g., "Master's" or "Male"
posterior_interval$Predictor <- posterior_interval$Predictor %>%
  gsub("Male", "Male", .) %>%
  gsub("Female", "Female", .) %>%
  gsub("Master s", "Master's", .) %>% # Correct possessive forms
  gsub("Doctorate", "Doctorate", .) %>%
  gsub("High School", "High School", .)

# Create the credibility interval plot
ggplot(posterior_interval, aes(x = reorder(Predictor, Estimate), y = Estimate)) +
  geom_point(size = 3, color = "blue") +
  geom_errorbar(aes(ymin = Lower, ymax = Upper), width = 0.2, color = "darkgray") +
  labs(
    title = "Credibility Intervals for Model Coefficients",
    x = "Predictors",
    y = "Estimate (90% CI)"
  ) +
  theme_minimal(base_size = 18) +
  theme(
    axis.text.y = element_text(size = 18),
    plot.title = element_text(size = 18, face = "bold")
  ) +
  coord_flip()
```
@fig-modelresults1 shows that the treatment effect for analytical thinking, bullshit receptivity, and conspiracy belief is statistically significant. Demographic predictors such as education and political ideology also show significant relationships with these psychological measures.

### Interpretation and Diagnostics
The findings confirm the effectiveness of psychological inoculation in enhancing analytical thinking and reducing susceptibility to bullshit receptivity and conspiracy belief. The inclusion of demographic covariates ensures robust estimation of treatment effects, controlling for confounding factors.

Model diagnostics, including trace plots and posterior predictive checks, confirm convergence and good model fit. These diagnostics are available in Appendix @sec-model-details.


# Discussion {#sec-discussion}

## Effects of Psychological Inoculation on Analytical Thinking and Beliefs
The results of the analysis provide evidence that psychological inoculation interventions significantly enhance analytical thinking and reduce susceptibility to conspiracy beliefs and receptivity to pseudo-profound statements. These findings support the hypothesis that exposing individuals to weakened forms of misinformation (inoculation) primes their cognitive defenses, making them better equipped to discern misinformation. Analytical thinking, in particular, showed a notable positive association with treatment, indicating that participants who received the inoculation intervention exhibited improved critical-thinking abilities. This reinforces prior research suggesting that cognitive training methods can effectively mitigate the influence of misleading information.

## Treatment Efficacy Across Demographics
The analysis show cases variation in treatment effects across demographic groups. Gender differences, although small, suggested that male participants responded less positively to the inoculation intervention than females. Educational background also played a role, with participants holding higher levels of education demonstrating stronger improvements in analytical thinking and reductions in belief in conspiracies. These findings are consistent with the idea that individuals with greater baseline cognitive skills or educational attainment may be more receptive to interventions designed to enhance critical-thinking. However, individuals with lower educational attainment may require additional or differently tailored interventions to achieve similar benefits.

## Limitations of the Inoculation Strategy
While the psychological inoculation approach demonstrated overall efficacy, several limitations merit discussion. First, the model does not account for potential reverse causality; individuals with higher baseline critical-thinking skills may be more likely to benefit from inoculation, leading to self-selection effects. Additionally, the observed impact on conspiracy beliefs, while statistically significant, was modest, suggesting that deeply entrenched beliefs may be resistant to change even with well-designed interventions. Future research should explore whether prolonged or repeated inoculation sessions yield stronger effects.

## Policy Implications and Recommendations
The findings have important implications for combating misinformation in public discourse. Policymakers and educators could integrate inoculation strategies into educational programs, particularly in contexts where misinformation poses significant societal risks, such as public health or political decision-making. For example, media literacy initiatives could adopt inoculation techniques to train individuals to identify common rhetorical strategies used in misinformation campaigns. Moreover, platforms hosting user-generated content might consider embedding inoculative messages into their user interfaces to preemptively reduce the spread of false information.

However, interventions must be carefully designed to account for varying levels of cognitive engagement and educational backgrounds across populations. Targeted approaches, such as simpler messaging for less educated groups or gamified interventions to increase engagement, may enhance the efficacy of these strategies.

## Weaknesses and Future Directions
The study faced several methodological limitations. Missing data, particularly in demographic variables, reduced the representativeness of the sample and limited the generalizability of the findings. Furthermore, the study did not address potential confounding factors, such as participants' preexisting exposure to misinformation or individual differences in cognitive bias susceptibility. Future studies should incorporate more exhaustive covariates to capture the complex interaction between demographic, psychological, and environmental factors.

Additionally, while the intervention successfully improved analytical thinking and reduced belief in misinformation, it remains unclear whether these effects translate to behavioral outcomes, such as sharing misinformation online. Experimental designs that measure behavioral responses directly would offer more practical evidence of the real-world applicability of psychological inoculation.

Finally, expanding the scope of the intervention to explore cross-cultural differences or applying it to different forms of misinformation (e.g., health-related versus political misinformation) could show the universality and adaptability of inoculation strategies. Combining inoculation with complementary interventions, such as fact-checking or debunking, could further strengthen its impact.

## Conclusion
The study demonstrates that psychological inoculation is a promising strategy for enhancing critical-thinking and reducing susceptibility to misinformation. While effective, it is not a panacea; additional research is needed to address its limitations and optimize its application in diverse contexts. The findings contribute to a growing body of evidence supporting interventions that empower individuals to navigate the complex and often misleading information landscape of the modern era.

\newpage

\appendix

# Model details {#sec-model-details}

## Posterior Predictive Checks
Posterior predictive checks were conducted to evaluate how well the Bayesian model fits the data. In @fig-pp-check, the posterior predictive distribution is compared to the observed data for technique discernment. The close alignment between the distributions suggests that the model captures the key features of the data effectively.

```{r}
#| echo: false
#| warning: false
#| message: false
#| include: false

model <- stan_glm(
  formula = technique_discernment ~ treatment + standardized_analytical_thinking + 
    standardized_conspiracy_belief + standardized_bullshit_receptivity + 
    gender + education + political_ideology + age_text,
  data = cleaned_data,
  family = gaussian(),
  prior = normal(location = 0, scale = 2.5, autoscale = TRUE),
  prior_intercept = normal(location = 0, scale = 2.5, autoscale = TRUE),
  prior_aux = exponential(rate = 1, autoscale = TRUE),
  seed = 853
)
```

```{r}
#| label: fig-pp-check
#| fig-cap: Posterior predictive checks for Bayesian model fit
#| echo: false
#| warning: false
#| message: false
#| fig-height: 8 
#| fig-width: 10

pp_check(model, plotfun = "dens_overlay") +
  labs(title = "Posterior Predictive Check", x = "Technique Discernment", y = "Density") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 20, face = "bold"),
    axis.text.x = element_text(size = 18),
    axis.text.y = element_text(size = 18),
    axis.title.y = element_text(size = 20),
    axis.title.x = element_text(size = 20)
  )
```
## Markov Chain Monte Carlo Convergence Check
To assess the convergence of the Markov chain Monte Carlo (MCMC) algorithm used in the Bayesian regression, trace plots were generated for all parameters. These trace plots, shown in @fig-trace-plots, demonstrate horizontal, oscillating patterns with no significant trends, suggesting that the chains have reached stationarity and mixed well.

```{r}
#| label: fig-trace-plots
#| fig-cap: "Trace plots for key model parameters"
#| fig-subcap: ["Intercept", "Treatment: Inoculation", "Standardized Analytical Thinking"]
#| echo: false
#| eval: true
#| warning: false
#| message: false
#| fig-height: 8 
#| fig-width: 10

# Create trace plots for specific parameters
trace_plot <- mcmc_trace(
  as.array(model), 
  pars = c("(Intercept)", "treatmentInoculation", "standardized_analytical_thinking"),
  facet_args = list(ncol = 1) # Arrange plots vertically
)

# Replace "_" with " " in parameter titles
trace_plot <- trace_plot +
  scale_y_discrete(labels = c(
    "(Intercept)" = "Baseline: Intercept",
    "treatmentInoculation" = "Treatment: Inoculation",
    "standardized_analytical_thinking" = "Standardized Analytical Thinking"
  )) +
  theme_minimal(base_size = 5) +
  labs(
    title = "Trace Plots for Key Model Parameters",
    x = "Iteration",
    y = "Parameter Values"
  ) +
  theme(
    plot.title = element_text(size = 20, face = "bold"),
    axis.title.x = element_text(size = 18),
    axis.title.y = element_text(size = 18),
    strip.text = element_text(size = 18),
    legend.text = element_text(size = 18)
  )

# Display the trace plot
trace_plot

```
In addition, the Rhat diagnostic was computed to assess convergence quantitatively. As shown in @fig-rhat, all Rhat values are close to 1, indicating good convergence across all model parameters.

```{r}
#| label: fig-rhat
#| fig-cap: "Rhat diagnostic for Bayesian model parameters"
#| echo: false
#| eval: true
#| warning: false
#| message: false
#| fig-height: 5
#| fig-width: 10

# Extract Rhat values
rhat_values <- bayesplot::rhat(model)  # This works directly with a fitted Bayesian model object

# Convert Rhat values to a data frame
rhat_data <- as.data.frame(rhat_values) %>%
  rownames_to_column(var = "Parameter") %>%
  rename(Rhat = rhat_values) %>%
  mutate(
    Parameter = gsub("_", " ", Parameter), # Replace underscores with spaces
    Parameter = stringr::str_replace_all(Parameter, "\\(Intercept\\)", "Baseline: Intercept"),
    Parameter = stringr::str_replace(Parameter, "treatmentInoculation", "treatment: Inoculation"),
    Parameter = stringr::str_replace(Parameter, "genderMale", "gender: Male"),
    Parameter = stringr::str_replace(Parameter, "genderNon-binary", "gender: Non-binary"),
    Parameter = stringr::str_replace(Parameter, "genderPrefer not to say", "gender: Prefer not to say"),
    Parameter = stringr::str_replace(Parameter, "educationDoctorate", "education: Doctorate"),
    Parameter = stringr::str_replace(Parameter, "educationHigh School", "education: High School"),
    Parameter = stringr::str_replace(Parameter, "educationMaster's", "education: Master's"),
    Parameter = stringr::str_replace(Parameter, "political ideologyLiberal", "political ideology: Liberal"),
    Parameter = stringr::str_replace(Parameter, "political ideologyModerate", "political ideology: Moderate"),
    Parameter = stringr::str_replace(Parameter, "age text25-34", "age: 25-34"),
    Parameter = stringr::str_replace(Parameter, "age text35-44", "age: 35-44"),
    Parameter = stringr::str_replace(Parameter, "age text45-54", "age: 45-54"),
    Parameter = stringr::str_replace(Parameter, "age text55 or older", "age: 55 or older")
  )

# Plot Rhat values
ggplot(rhat_data, aes(x = Rhat, y = reorder(Parameter, Rhat))) +
  geom_point(size = 3, color = "blue") +
  labs(
    title = "Rhat Diagnostic for Model Convergence",
    x = "Rhat Values",
    y = "Model Parameters"
  ) +
  theme_minimal(base_size = 14) +
  theme(
    plot.title = element_text(size = 18, face = "bold", hjust = 0.5),
    axis.text = element_text(size = 18),
    axis.title = element_text(size = 20)
  ) +
  geom_vline(xintercept = 1.1, linetype = "dashed", color = "red") # Highlight Rhat threshold

```
## Multicollinearity Assessment
Variance Inflation Factor (VIF) values were calculated to detect potential multicollinearity among predictors. All VIF values were below 5, indicating that multicollinearity is not a significant concern in the model. These results are summarized in Table @tbl-vif-values.

```{r}
#| label: tbl-vif-values
#| tbl-cap: Variance Inflation Factors (VIF) for Bayesian model predictors
#| echo: false
#| warning: false
#| message: false
#| fig-height: 5 
#| fig-width: 15

# Calculate VIF values for the model
vif_values <- vif(model)  # Replace 'model' with the actual model object name
    
vif_values %>%
  as.data.frame() %>%
  rownames_to_column("Predictor") %>%
  mutate(Predictor = gsub("_", " ", Predictor)) %>% # Replace underscores with spaces
  kable(
    booktabs = TRUE,
    caption = "VIF values for Bayesian regression predictors"
  ) %>%
  kable_styling(full_width = FALSE, position = "center")
```
## Credibility Intervals
The 90% credibility intervals for the model coefficients are visualized in @fig-credibility-intervals. These intervals highlight the uncertainty around the estimated effects for each predictor. Notably, the treatmentInoculation variable has a positive and significant effect, providing evidence for the effectiveness of psychological inoculation interventions.

\newpage

```{r}
#| label: fig-credibility-intervals
#| fig-cap: "Credibility intervals for Bayesian model coefficients"
#| echo: false
#| eval: true
#| warning: false
#| message: false
#| fig-height: 5 
#| fig-width: 10

posterior_interval %>%
  mutate(Predictor = factor(Predictor, levels = Predictor[order(Estimate)])) %>%
  ggplot(aes(x = Predictor, y = Estimate)) +
  geom_point(size = 3, color = "blue") +
  geom_errorbar(aes(ymin = Lower, ymax = Upper), width = 0.2, color = "darkgray") +
  labs(
    title = "90% Credibility Intervals for Model Coefficients",
    x = "Predictors",
    y = "Estimate"
  ) +
  theme_minimal() +
  coord_flip() +
  theme(
    plot.title = element_text(size = 20, face = "bold"),
    axis.text.x = element_text( size = 18),
    axis.text.y = element_text(size = 18),
    axis.title.y = element_text(size = 20),
    axis.title.x = element_text(size = 20)
  )

```

## Appendix: Full Model Summary
A full summary of the Bayesian model results, including estimates, standard errors, and credible intervals, is provided in Appendix @tbl-model-summary.

```{r}
#| label: tbl-model-summary
#| tbl-cap: Detailed summary of Bayesian regression model
#| echo: false
#| eval: true
#| warning: false
#| message: false
#| fig-height: 5 
#| fig-width: 15

# Update labels for predictors
model_summary <- model_summary %>%
  dplyr::mutate(
    term = stringr::str_replace_all(term, "_", " "), # Replace underscores with spaces
    term = stringr::str_replace_all(term, "\\(Intercept\\)", "Baseline: Intercept"), # Format intercept
    term = stringr::str_replace(term, "treatmentInoculation", "treatment: Inoculation"),
    term = stringr::str_replace(term, "genderMale", "gender: Male"), # Format gender terms
    term = stringr::str_replace(term, "genderNon-binary", "gender: Non-binary"),
    term = stringr::str_replace(term, "genderPrefer not to say", "gender: Prefer not to say"),
    term = stringr::str_replace(term, "educationDoctorate", "education: Doctorate"), # Format education terms
    term = stringr::str_replace(term, "educationHigh School", "education: High School"),
    term = stringr::str_replace(term, "educationMaster's", "education: Master's"),
    term = stringr::str_replace(term, "political ideologyLiberal", "political ideology: Liberal"), # Format ideology
    term = stringr::str_replace(term, "political ideologyModerate", "political ideology: Moderate"),
    term = stringr::str_replace(term, "age text25-34", "age: 25-34"), # Format age groups
    term = stringr::str_replace(term, "age text35-44", "age: 35-44"),
    term = stringr::str_replace(term, "age text45-54", "age: 45-54"),
    term = stringr::str_replace(term, "age text55 or older", "age: 55 or older")
  )

# Generate and display the table with formatted labels
model_summary %>%
  dplyr::select(term, estimate, std.error, conf.low, conf.high) %>%
  dplyr::mutate(
    conf.int = paste0("(", round(conf.low, 2), ", ", round(conf.high, 2), ")") # Format confidence intervals
  ) %>%
  dplyr::select(term, estimate, std.error, conf.int) %>%
  kbl(
    caption = "Detailed Bayesian regression model summary with formatted predictor labels",
    col.names = c("Predictor", "Estimate", "Standard Error", "95% CI")
  ) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),
    full_width = FALSE
  )
```

## Interpretation and Implications
The diagnostics and results presented here confirm that the Bayesian model is well-specified, converged, and fits the data appropriately. The findings support the theoretical underpinnings of psychological inoculation and its impact on critical-thinking and misinformation resistance.
\newpage


# References


